...
export default function ProjectGrid({ proyectos }: Props) {
  ...
  function openLightbox(p: typeof source[0]) {
    setLightbox({
      imagen: p.imagen_portada || '/fotos/proyecto-facultad-artes.jpg',
      titulo: p.titulo,
      area_m2: p.area_m2,
      anio: p.anio,
      tipologia: p.tipologia.charAt(0).toUpperCase() + p.tipologia.slice(1),
    })
  }
    ...
    <>
          ...
          {filtered.map((project, index) => {
            ...
            return (
              <button
                ...
                className={`group relative overflow-hidden block text-left cursor-zoom-in ${
              ...
              >
                ...
                <div className="absolute inset-0 bg-dark-olive/0 group-hover:bg-dark-olive/70 transition-all duration-300" />
      ...
      {lightbox && (
        ...
        >
          ...
          <div
            className="relative w-full max-w-5xl max-h-[85vh] cursor-default"
          ...
          >
            ...
            <div className="mt-4 text-center">
              <h3 className="font-heading font-bold text-xl text-white">{lightbox.titulo}</h3>
              <p className="font-sans text-sm text-white/50 mt-1">
                {lightbox.tipologia}
                {lightbox.area_m2 ? ` · ${lightbox.area_m2.toLocaleString()} m²` : ''}
                {lightbox.anio ? ` · ${lightbox.anio}` : ''}
              </p>
            </div>
          </div>
        </div>
